...
describe('Router', () => {
  describe('Withdraw Initial Balance', () => {
    it('should emit Withdrew on withdraw initial balance', async function () {
      ...
      await expect(routerContract.withdrawInitialBalance(nodeAddress, amount))
        .to.emit(routerContract, 'RegistryWithdrew')
        .withArgs(registryAddress, anyValue, amount)
    })

    it('should revert if try to withdraw initial balance of a non node manager ', async function () {
      const { fakeStrategyAddress } = await loadFixture(fakeStrategySingle.deployFakeStrategySingleFixture)
      ...
      await expect(
        otherAccountRouterInstance.withdrawInitialBalance(nodeAddress, amount)
      ).to.be.revertedWithCustomError(otherAccountRouterInstance, 'NotNodeManager()')
    })
  })
})
